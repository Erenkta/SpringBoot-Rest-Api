...
public class BookManagerTest {
	...
	public void testThatListBookReturnsListWhenBooksExists() {
		...
		//When'in amacı eğer .... çağırılırsa ... dön demek gibi
		final List<BookDto> result = testService.listBooks();
	...
	public void testThatBookExistsReturnsFalseWhenBookDoesntExist() {
		...
		assertEquals(false,testService.isBookExists(TestData.testBookDto()));
	}
	@Test
	public void testThatBookExistsReturnsTrueWhenBookExists() {
		when(bookRepository.existsById(any())).thenReturn(true);
		assertEquals(true,testService.isBookExists(TestData.testBookDto())); 
	}
	@Test
	public void testThatDeleteBookDeletesBook() {
		final String isbn = "000000";
		testService.deleteBookById(isbn);
		verify(bookRepository,times(1)).deleteById(eq(isbn)); // eq argüman vermeye yarıyor diyebiliriz
	}
